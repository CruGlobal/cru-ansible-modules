...
def wait_for_status(vdb,vstatus):
    ...
    while not all(item == vstatus for itme in get_db_status(vdb)) and datetime.now() < vduration:
        time.sleep(2)
...
def main ():
  ...
  # if all nodes are not already the state we're looking for execute the command
  if not all(item == vstatus for itme in get_db_status(vdb)): # current_status != exp_sstatus:
